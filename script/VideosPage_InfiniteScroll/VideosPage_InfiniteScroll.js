...
async function REMOVE_VIDEO() {
  ...
  const videos = mediaGrid.querySelectorAll("div[INFINTE_SCROLL_ITEM]");
...
BLOCK_REMOVE_INTERVAL=setInterval(async ()=>{await REMOVE_VIDEO();},20);
    ...
    items.forEach(node => {
      ...
      if (loadedIDsVideos.has(node.id)) return

      loadedIDsVideos.add(node.id)
      existingUrls.add(itemUrl)

      const card = createVideoTile({
...
function createVideoTile (item) {
  ...
  div.setAttribute('INFINTE_SCROLL_ITEM', 'oh hello there')
  div.innerHTML = `
        <a class="hover-overlay relative block overflow-hidden rounded before:z-10 before:rounded"
            ...
            <span class="pointer-events-none absolute top-1/2 left-1/2 z-10 flex size-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-stroke-subdued bg-translucent-dark-600 text-neutral-strong"><svg viewBox="0 0 24 24" role="presentation" style="width: 1.5rem; height: 1.5rem;"><path d="M8,5.14V19.14L19,12.14L8,5.14Z" style="fill: currentcolor;"></path></svg></span>
        </a>
