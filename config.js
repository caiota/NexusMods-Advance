...
document.querySelector("div#DeleteAll_HiddenMods").addEventListener("click", function () {
   chrome.runtime.sendMessage(
    {
      action: "Delete_HiddenMods"
    },
    function (response) {
...
document.querySelector("div#overflow").addEventListener("click", function () {
  if (document.querySelector("div#ImportConfig_Panel").style.display === "block") {
    window.location.reload();
  }
  document.querySelector("div#overflow").style.display = "none";
  document.querySelector("div#CDN_speed_PopUp").style.display = "none";
  document.querySelector("div#ImportConfig_Panel").style.display = "none";
...
async function ProcessPosts(thread, mod_id, game_id, page) {
  ...
  const response = await fetch(
    "https://www.nexusmods.com/Core/Libs/Common/Widgets/CommentContainer?RH_CommentContainer=search_text:,tabbed=1,object_id=" +
      mod_id +
      ",game_id=" +
      game_id +
      ",object_type=1,thread_id=" +
      thread +
      ",skip_opening_post=0,user_is_blocked=,searchable=false,page_size:10,page:" +
      page,
    {
      headers: {
        accept: "text/html, */*; q=0.01",
  ...
  doc
    .querySelector("ol")
    .querySelectorAll("div.comment-actions")
    .forEach((e) => {
      if (e) {
        e.remove();
      }
  ...
  doc
    .querySelector("ol")
    .querySelectorAll("img.emoticon")
    .forEach((e) => {
      if (e) {
        e.src = "https://www.nexusmods.com" + e.getAttribute("src");
      }
    });
  if (doc.querySelector("ol") && doc.querySelector("ol").childElementCount > 0) {
    if (page == 1) {
      ...
      doc
        .querySelector("ol")
        .querySelectorAll("li.comment-sticky")
        .forEach((e) => {
          if (e) {
            e.remove();
          }
...
document.querySelector("div#viewBugs").addEventListener("click", async function (ev) {
  ...
  if (doc.querySelector("table.forum-bugs") && doc.querySelector("table.forum-bugs").childElementCount > 0) {
    doc
      .querySelector("table.forum-bugs")
      .querySelectorAll("span.table-inline-hidden")
      .forEach((e) => {
        if (e) {
          e.remove();
        }
    ...
    doc
      .querySelector("table.forum-bugs")
      .querySelectorAll("td.table-bug-priority")
      .forEach((e) => {
        if (e) {
          e.remove();
        }
    ...
    doc
      .querySelector("table.forum-bugs")
      .querySelectorAll('span[id^="issueFixedInVersion"]')
      .forEach((e) => {
        if (e) {
          e.style.color = "#44f244";
    ...
    document
      .querySelector("div#Bugs")
      .querySelectorAll("a.issue-title:not([eventAdded])")
      .forEach((e) => {
        if (e) {
          e.setAttribute("eventAdded", true);
          e.removeAttribute("onclick");
          e.removeAttribute("href");
          e.addEventListener("click", async function (ev) {
            await loadIssueReplies(ev);
          });
        }
      });
    stillLoading = false;
...
async function FetchAndAdd(mod_id, game_id, page) {
  ...
  const response = await fetch(
    "https://www.nexusmods.com/Core/Libs/Common/Widgets/ModBugsTab?RH_ModBugsTab=issue_id:,id=" +
      mod_id +
      ",game_id=" +
      game_id +
      ",priority:-1,status:-1,sort_by:date,order:DESC,page_size:10,page:" +
      page,
    {
      headers: {
        accept: "text/html, */*; q=0.01",
...
document.querySelector("img#nexusIcon").addEventListener("click", function () {
  window.open("https://www.nexusmods.com/site/mods/1018");
});
document.querySelector("div#moreOptions span#more_OfficialPost").addEventListener("click", function () {
  window.open("https://www.nexusmods.com/site/mods/1018");
});
document.querySelector("div#moreOptions span#more_Export_Config").addEventListener("click", function () {
  chrome.runtime.sendMessage(
    {
      action: "ExportConfig",
    },
    function (response) {
      if (response && response.success) {
        ...
        a.download = `NexusModsAdvance_${MOD_VERSION}_Backup_${new Date().toISOString().slice(0, 10)}.json`;
...
document.querySelector("div#moreOptions span#more_Import_Config").addEventListener("click", function () {
  document.querySelector("div#overflow").style.display = "block";
  document.querySelector("div#ImportConfig_Panel").style.display = "block";

  document.getElementById("importFile").addEventListener("change", loadBackupFile);
});
let ImportedBackupData = null;
...
async function loadBackupFile(event) {
  ...
  try {
    ...
    ImportedBackupData = backup.data;

    document.getElementById("BackupVersion").textContent = backup.extension_version ?? "Unknown";

    document.getElementById("BackupBrowser").textContent = backup.browser ?? "Unknown";

    document.getElementById("BackupDate").textContent = backup.exportedAt ? new Date(backup.exportedAt).toLocaleString() : "Unknown";

    ImportMsg.style.color = "#3fb950";
    ImportMsg.textContent = translate_strings.ImportResults_Success.message;

    console.log("Imported Backup Data:");
    console.log(ImportedBackupData);
    if (ImportedBackupData) {
      chrome.runtime.sendMessage(
        {
          action: "ImportConfig",
          backupItems: ImportedBackupData,
        },
        function (response) {
          if (response && response.success) {
            ...
            ImportMsg.style.color = "#ff4d4d";
            ImportMsg.innerHTML = translate_strings.ImportResults_Failure.message + "<br>err: " + err;
          }
        },
      );
    }
...
async function loadIssueReplies(ev) {
  ...
  doc.querySelectorAll("div.comment-actions").forEach((e) => {
    if (e) {
      e.remove();
    }
  });
                ...
                async function (response3) {
                  ...
                  if (options["Endorsed"] == false) {
                    chrome.runtime.sendMessage(
                      ...
                      async function (response3) {
                        if (response3.message == true && response3.success) document.querySelector("div#PleaseEndorse").style.display = "block";
                      },
                    );
                  }
                },
